...
public class Yolov5TFLiteDetector {
    ...
    private final int[] OUTPUT_SIZE = new int[]{1, 25200, 31};
    private Boolean IS_INT8 = false;
    private final float DETECT_THRESHOLD = 0.5f;
    private final float IOU_THRESHOLD = 0.75f;
    private final float IOU_CLASS_DUPLICATED_THRESHOLD = 0.7f;
    private final String MODEL_YOLOV5S = "702010_gpua100.tflite";
    private final String LABEL_FILE = "classes.txt";
    MetadataExtractor.QuantizationParams input5SINT8QuantParams = new MetadataExtractor.QuantizationParams(0.0f, 0);
    MetadataExtractor.QuantizationParams output5SINT8QuantParams = new MetadataExtractor.QuantizationParams(0.0f, 0);
    ...
    public ArrayList<Recognition> detect(Bitmap bitmap) {
        ...
        // yolov5s-tflite的输入是:[1, 640, 640,3], 摄像头每一帧图片需要resize,再归一化
        ...
        // yolov5s-tflite的输出是:[1, 25200, 31], 可以从v5的GitHub release处找到相关tflite模型, 输出是[0,1], 处理到640.
